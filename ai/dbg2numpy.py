...
def type_name_to_numpy_primitive(typename, structured_primitive=False):
    ...
    numpy_primitives = {
        ...
        'long':               'np.int64',  # on win64 should be np.int32
        'unsigned long':      'np.uint64', # on win64 should be np.uint32
        'long long':          'np.int64',
...
def generate_numpy_module(variables_names, binary_path, module_path=None):
    ...
    # This dbg2numpy is running under the venv, so we get its
    # site-packages path in order to provide it to the script, which
    # is executed under GDB. GDB uses its own Python, so it needs to
    # load our venv packages, like `numpy`, otherwise `No module named
    # 'numpy'` exception
    this_venv_site_packages = sysconfig.get_paths().get("purelib")
