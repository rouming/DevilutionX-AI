...
def get_environment(d, radius=None, show_invisible=False, show_unexplored=False):
...
def get_dungeon_graph_and_path(d, start, goal):
    ...
    env = get_environment(d, show_invisible=True, show_unexplored=True)
    # 0 - walls
...
class DiabloGame:
    def __init__(self, state_dir, proc, log_file, mshared_path,
                 mmapped, state, game_ticks_per_step):
    ...
    def run(config):
        ...
        return DiabloGame(state_dir, proc, log_file, mshared_path,
                          mmapped, state, game_ticks_per_step)
    ...
    def attach(config):
        ...
        game_ticks_per_step = config["game-ticks-per-step"]
        mmapped, state = map_shared_state(mshared_path, offset)
        return DiabloGame(None, None, None, mshared_path, mmapped,
                          state, game_ticks_per_step)
