...
class Sprout:
    ...
    def create(self,
        ...
        Behavior:
          - If from_head is provided and head is NOT provided, create
            acts as a snapshot operation only: it snapshots the active
            head (inserts a node) and returns the snapshot id.
          - Otherwise, it creates a new run whose parent is:
              * if from_run provided: from_run (if that run is active
                it will be snapshotted first, and parent becomes the
                snapshot id),
              * if from_head provided: snapshot of that head (head is
                snapshotted first),
        ...
        # if the source run exists and is active (any head points to
        # it), snapshot it first
        parent_for_new = None
        if src_run:
    ...
    def rename(self, old_head: str, new_head: str):
        ...
        meta = self._load_meta()
        heads = meta.get("heads", {})
        if old_head not in heads:
            raise SproutError(f"head '{old_head}' not found")
...
def cli_rename(args, sprout: Sprout) -> int:
    try:
        sprout.rename(args.old_head, args.new_head)
    except Exception as e:
        print(f"ERROR: {e}", file=sys.stderr)
        return 2
    print(f"Renamed head '{args.old_head}' -> '{args.new_head}'")
    return 0
...
def build_parser(prog, suppress_working_dir=False, add_help=True):
    ...
    # create
    pc = sub.add_parser("create", help="Create a run within a group or from a specified run or head.")
    ...
    # remove
    pr = sub.add_parser("remove", help="Remove the whole group, run or head")
    ...
    # edit
    pe = sub.add_parser("edit", help="Edit meta info for specified run or head")
    ...
    pr.add_argument("new_head", help="New head name")
...
class SproutCLITests(unittest.TestCase):
    ...
    def test_rename(self):
        ...
        # rename A -> B
        rc, out, err = self.run_sprout("rename A B")
        self.assertEqual(rc, 0, msg=err)
        ...
        # rename NONEXIST -> B
        rc, out, err = self.run_sprout("rename NONEXIST B")
        self.assertEqual(rc, 2, msg=err)
        ...
        # rename B -> B
        rc, out, err = self.run_sprout("rename B B")
        self.assertEqual(rc, 2, msg=err)
...
#   or
#
# ./sprout.py test SproutCLITests.<testname>
#
